...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QMessageBox, QRadioButton, QButtonGroup, QComboBox, QFrame,
...
class PaperSelectionDialog(QDialog):
    ...
    def init_ui(self) -> None:
        ...
        for paper_key, title, authors, year in self.finished_papers:
            ...
            item.setData(Qt.ItemDataRole.UserRole, paper_key)  # Store paper_key for retrieval
    ...
    def on_select(self) -> None:
        ...
        if current_item is None:
            QMessageBox.warning(self, "No Selection", "Please select a paper from the list.")
            return
...
class DataExtractionGUI(QMainWindow):
    ...
    def __init__(self, user: str, json_file: str = "data-items.json", csv_file: str = "2026-02-24_data-extraction-assignments(in).csv") -> None:
        ...
        self.toggle_buttons: Dict[str, QCheckBox] = {}  # Toggle buttons for special options
        self.toggle_line_edits: Dict[str, QLineEdit] = {}  # Line edits for toggle-associated text
        self.toggle_states: Dict[str, Dict[str, Dict[str, bool]]] = {}  # Track toggle states: entry_key -> question_key -> attribute -> bool
        self.toggle_texts: Dict[str, Dict[str, Dict[str, str]]] = {}  # Track toggle text: entry_key -> question_key -> attribute -> str
        self.question_tabs: Optional[QTabWidget] = None  # Reference to nested question tabs
        self.excluded_papers: Dict[str, bool] = {}  # Track which papers are excluded from full text review
        self.excluded_reasons: Dict[str, str] = {}  # Track exclusion reasons for each paper
        self.exclude_checkbox: Optional[QCheckBox] = None  # Reference to the exclude checkbox
        self.exclude_reason_input: Optional[QLineEdit] = None  # Reference to the exclusion reason text field
    ...
    def load_csv_data(self) -> bool:
        ...
        try:
            ...
            if not self.papers:
                print(f"Error: No papers assigned to user '{self.user}' in {self.csv_file}", file=sys.stderr)
                return False
        ...
        if os.path.exists(session_file):
            try:
                with open(session_file, 'r') as f:
                    ...
                    if session_data.get('user') == self.user and 'current_paper_index' in session_data:
    ...
    def on_go_back(self) -> None:
        ...
        if not finished_papers:
            QMessageBox.information(self, "No Finished Papers", "There are no previously finished papers to go back to.")
            return
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if selected_key:
                ...
                try:
                    ...
                    QMessageBox.warning(self, "Error", "Could not find the selected paper.")
    ...
    def init_ui(self) -> None:
        ...
        self.paper_info_label.setStyleSheet("background-color: #2c3e50; color: #ecf0f1; padding: 10px; border-radius: 5px; border: 1px solid #1a252f;")
        self.paper_info_label.setWordWrap(True)
        self.paper_info_label.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse | Qt.TextInteractionFlag.TextSelectableByKeyboard)
        paper_info_layout.addWidget(self.paper_info_label)
        
        # Exclude checkbox
        self.exclude_checkbox = QCheckBox("⚠️ Exclude from full text review (irrelevant for research questions)")
        exclude_font = QFont()
        exclude_font.setPointSize(9)
        exclude_font.setBold(True)
        self.exclude_checkbox.setFont(exclude_font)
        self.exclude_checkbox.setStyleSheet("QCheckBox { color: #d9534f; padding: 5px; }")
        ...
        self.exclude_reason_input.setPlaceholderText("e.g., 'Out of scope', 'Duplicate', 'Irrelevant methodology'")
        ...
        self.exclude_reason_label = reason_label  # Store reference for toggling visibility
    ...
    def load_paper(self, index: int) -> None:
        ...
        if index >= len(self.paper_keys):
            QMessageBox.information(self, "Completed", "All papers have been processed!")
            self.close()
            return
        ...
        if self.question_tabs is not None:
            for question_key, options_dict in self.data.items():
                question_tab = self.create_question_tab(entry_key, question_key, options_dict)
    ...
    def _load_paper_progress(self, entry_key: str) -> None:
        ...
        self.excluded_papers[entry_key] = paper_data.get('excluded_from_full_text_review', False)
            ...
            for attribute, selections in responses[question_key].items():
                ...
                for selection in selections:
                    if selection.startswith("Other: "):
                        ...
                        self.selected_values[entry_key][question_key][attribute].append("Other")
                        self.selected_Other_text[entry_key][question_key][attribute] = Other_text
                    elif selection.startswith("Discussion needed: "):
                        # Extract discussion text
                        discussion_text = selection[19:]  # Remove "Discussion needed: " prefix
                        self.selected_values[entry_key][question_key][attribute].append("Discussion needed")
                        ...
                        if attribute not in self.selected_Other_text[entry_key][question_key]:
                            self.selected_Other_text[entry_key][question_key][attribute] = ""
                        ...
                        self.selected_values[entry_key][question_key][attribute].append(selection)
    ...
    def _save_session_state(self) -> None:
        ...
        try:
            session_data = {
                ...
                'current_paper_key': self.paper_keys[self.current_paper_index] if self.current_paper_index < len(self.paper_keys) else None
            }
    ...
    def finish_paper(self) -> None:
        ...
        violations = []
        try:
            violations = sanity_checks.validate_paper(paper_entry, config_path=str(pathlib.Path('sanity_checks.json')))
        except Exception as e:
            # If the validator fails unexpectedly, surface an error and abort finishing so user can investigate
            QMessageBox.critical(self, "Sanity Check Error", f"An error occurred while running sanity checks: {str(e)}")
            return
    ...
    def validate_all_required_fields(self) -> bool:
        ...
        for question_key, options_dict in self.data.items():
            ...
            for attribute in options_dict.keys():
                ...
                selections = self.selected_values[entry_key].get(question_key, {}).get(attribute, [])
                ...
                if "Discussion needed" in selections:
                    ...
                    if discussion_key in self.discussion_text_inputs:
                        discussion_text = self.discussion_text_inputs[discussion_key].text().strip()
    ...
    def create_question_tab(self, entry_key: str, question_key: str, options_dict: Dict[str, List[str]]) -> QWidget:
        ...
        for attribute, options in options_dict.items():
            ...
            if isinstance(options, dict):
                # New format with "options" and optionally "toggle_option"
            ...
            display_options = [opt for opt in display_options_base if opt not in ("single-choice", "Multiple")]
            ...
            if "Other" in display_options_base:
                ...
                text_label.setStyleSheet("color: #555555; font-weight: bold; padding: 5px 0px;")
                ...
                text_input.textChanged.connect(
                    lambda text, e=entry_key, q=question_key, a=attribute:
                    self.on_Other_text_changed(e, q, a, text)
                )
            ...
            if toggle_config:
                ...
                toggle_button.setStyleSheet("QCheckBox { color: #1976d2; font-weight: bold; padding: 5px 0px; }")
                toggle_button.stateChanged.connect(
                    lambda state, e=entry_key, q=question_key, a=attribute:
                    self.on_toggle_changed(e, q, a, state)
                )
                ...
                toggle_text_input.textChanged.connect(
                    lambda text, e=entry_key, q=question_key, a=attribute:
                    self.on_toggle_text_changed(e, q, a, text)
                )
    ...
    def _restore_ui_state(self, layout: QVBoxLayout, entry_key: str, question_key: str) -> None:
        ...
        Args:
            ...
            question_key (str): The research question identifier.
        """
        if entry_key not in self.selected_values or question_key not in self.selected_values[entry_key]:
            return
        ...
        for attribute in self.selected_values[entry_key][question_key]:
            ...
            if "Discussion needed" in selections:
                ...
                if discussion_key in self.discussion_text_inputs:
                    ...
                    if hasattr(self, '_discussion_texts') and discussion_key in self._discussion_texts:
            ...
            if entry_key in self.toggle_states and question_key in self.toggle_states[entry_key]:
                enabled = self.toggle_states[entry_key][question_key].get(attribute, False)
                ...
                if toggle_text_key in self.toggle_line_edits:
                    ...
                    existing_text = self.toggle_texts.get(entry_key, {}).get(question_key, {}).get(attribute, "")
                    if existing_text:
    ...
    def _create_checkboxes(self, layout: QVBoxLayout, entry_key: str, question_key: str, 
                          attribute: str, options: List[str]) -> None:
        ...
        for option in options:
            ...
            checkbox.stateChanged.connect(
                lambda state, e=entry_key, q=question_key, a=attribute, o=option:
                self.on_checkbox_changed(e, q, a, o, state)
            )
    ...
    def _create_multiple_selection_widget(self, layout: QVBoxLayout, entry_key: str, question_key: str,
                                         attribute: str, options: List[str]) -> None:
        ...
        add_btn.clicked.connect(
            lambda checked=False, e=entry_key, q=question_key, a=attribute:
            self.on_multiple_add_value(e, q, a)
        )
        ...
        selected_container.setStyleSheet("QWidget { background-color: #f9f9f9; border: 1px solid #e0e0e0; border-radius: 3px; padding: 8px; }")
    ...
    def on_multiple_add_value(self, entry_key: str, question_key: str, attribute: str) -> None:
        ...
        if not selected_text or selected_text == "-- Select an option --":
            QMessageBox.warning(None, "No Selection", "Please select an option from the dropdown.")
            return
        ...
        if selected_text in self.selected_values[entry_key][question_key][attribute]:
            QMessageBox.information(None, "Already Selected", f"'{selected_text}' is already in your selection.")
            return
    ...
    def _update_multiple_selection_display(self, entry_key: str, question_key: str, attribute: str) -> None:
        ...
        selections = self.selected_values.get(entry_key, {}).get(question_key, {}).get(attribute, [])
        ...
        if not selections:
            ...
            for value in selections:
                ...
                delete_btn.clicked.connect(
                    partial(self.on_multiple_remove_value, entry_key, question_key, attribute, value)
                )
    ...
    def on_multiple_remove_value(self, entry_key: str, question_key: str, attribute: str, value: str) -> None:
        ...
        """
        if (entry_key in self.selected_values and 
            question_key in self.selected_values[entry_key] and
            attribute in self.selected_values[entry_key][question_key] and
            value in self.selected_values[entry_key][question_key][attribute]):
            
            self.selected_values[entry_key][question_key][attribute].remove(value)
            self._update_multiple_selection_display(entry_key, question_key, attribute)

    def _create_radio_buttons(self, layout: QVBoxLayout, entry_key: str, question_key: str,
                             attribute: str, options: List[str]) -> None:
        ...
        for idx, option in enumerate(options):
            ...
            radio.toggled.connect(
                lambda checked, e=entry_key, q=question_key, a=attribute, o=option:
                self.on_radio_button_changed(e, q, a, o, checked)
            )
    ...
    def _create_dropdown_widget(self, layout: QVBoxLayout, entry_key: str, question_key: str,
                               attribute: str, options: List[str]) -> None:
        ...
        combo.currentTextChanged.connect(
            lambda text, e=entry_key, q=question_key, a=attribute:
            self.on_dropdown_changed(e, q, a, text)
        )
        self.comboboxes[combo_key] = combo
    ...
    def _add_discussion_field(self, layout: QVBoxLayout, entry_key: str, question_key: str, attribute: str) -> None:
        ...
        discussion_input.textChanged.connect(
            lambda text, e=entry_key, q=question_key, a=attribute, k=discussion_key:
            self.on_discussion_text_changed(e, q, a, k, text)
        )
    ...
    def on_checkbox_changed(self, entry_key: str, question_key: str, attribute: str, option: str, state: int) -> None:
    ...
    def on_radio_button_changed(self, entry_key: str, question_key: str, attribute: str, option: str, checked: bool) -> None:
    ...
    def on_dropdown_changed(self, entry_key: str, question_key: str, attribute: str, text: str) -> None:
    ...
    def on_discussion_text_changed(self, entry_key: str, question_key: str, attribute: str, discussion_key: str, text: str) -> None:
    ...
    def on_Other_text_changed(self, entry_key: str, question_key: str, attribute: str, text: str) -> None:
    ...
    def on_toggle_changed(self, entry_key: str, question_key: str, attribute: str, state: int) -> None:
        ...
        if toggle_text_key in self.toggle_line_edits:
            ...
            if is_checked:
                ...
                existing = self.toggle_texts.get(entry_key, {}).get(question_key, {}).get(attribute, "")
                if existing:
                ...
                text_input.clear()
                # clear stored text
                if entry_key in self.toggle_texts and question_key in self.toggle_texts[entry_key]:
                    self.toggle_texts[entry_key][question_key][attribute] = ""

    def on_toggle_text_changed(self, entry_key: str, question_key: str, attribute: str, text: str) -> None:
        ...
        try:
            ...
            for entry_key in self.selected_values:
                ...
                output[entry_key] = {
                    ...
                    'excluded_from_full_text_review': self.excluded_papers.get(entry_key, False),
                ...
                for question_key in self.selected_values[entry_key]:
                    ...
                    for attribute in self.selected_values[entry_key][question_key]:
                        selections = self.selected_values[entry_key][question_key][attribute].copy()
                        
                        # Handle "Other" option
                        if "Other" in selections and self.selected_Other_text[entry_key][question_key][attribute]:
                            selections.remove("Other")
                            selections.append(f"Other: {self.selected_Other_text[entry_key][question_key][attribute]}")
                        
                        # Handle "Discussion needed" option
                        if "Discussion needed" in selections:
                            discussion_key = f"{entry_key}_{question_key}_{attribute}_discussion"
                            if discussion_key in self.discussion_text_inputs:
                                discussion_text = self.discussion_text_inputs[discussion_key].text()
                                if discussion_text:
                                    selections.remove("Discussion needed")
                                    selections.append(f"Discussion needed: {discussion_text}")
                        
                        output[entry_key]['responses'][question_key][attribute] = selections
                        ...
                        for attr, enabled in attrs.items():
                            if enabled:
                                if qk not in toggle_out:
                                    toggle_out[qk] = {}
                                text = self.toggle_texts.get(entry_key, {}).get(qk, {}).get(attr, "")
